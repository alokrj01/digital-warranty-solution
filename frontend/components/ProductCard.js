...
export default function ProductCard({ product }) {
  ...
  const statusColor = isExpired ? 'text-red-600 bg-red-50 border-red-100' : 
                      isExpiringSoon ? 'text-orange-600 bg-orange-50 border-orange-100' : 
  ...
  return (
    <div className="group bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
      ...
      {/* Status Badge */}
      <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${statusColor}`}>
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center border border-slate-100 group-hover:scale-110 transition-transform duration-500">
           <span className="text-xl font-black text-slate-300 uppercase">{product.name[0]}</span>
        </div>
      ...
      <div className="space-y-3 mb-6">
        <div className="flex items-center text-sm text-slate-500 font-medium">
          ...
          Expires: <span className="text-slate-900 ml-1 font-bold">{product.expiry_date}</span>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
            <div 
              ...
              style={{ width: isExpired ? '0%' : `${Math.min((daysLeft / 365) * 100, 100)}%` }}
            />
          </div>
        </div>
      </div>
